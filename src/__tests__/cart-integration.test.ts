...
describe('Cart Integration', () => {
  ...
  it('should apply discount code correctly', () => {
    const { addItem, applyDiscount, getTotal } = getStore();

    addItem({ id: 'item-1', name: 'Item', image: 'https://example.com/item.jpg', service: 'wash', price: 100.00, category: 'test' }, 1);

    const DISCOUNT_CODE = process.env.DISCOUNT_CODE || 'PROMO10';

    const before = getTotal();
    applyDiscount(DISCOUNT_CODE);    applyDiscount(DISCOUNT_CODE);
    const after = getTotal();

    expect(after).toBeLessThan(before);xpect(after).toBeLessThan(before);
  }););
});});
